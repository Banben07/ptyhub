...
所有快捷键都是 leader 前缀键加一个键。默认 leader 是 `Ctrl+\`，和 `ptyhub attach` 的脱离前缀一致。
...
不占用浏览器的 `Ctrl/Cmd+N`、`+W`、`+T`、`+K`，也不抢终端里的 `Ctrl+C`、`Ctrl+D`、`Ctrl+R`。整份键位存在 `~/.config/ptyhub/keymap.json`，在设置页的 Keyboard 标签里可改，leader 本身也能换。

**不要把 leader 设成 `Ctrl+Space`。** 那是 Windows、macOS、Linux 上输入法的中英文切换键；对于用中日韩输入法的人，这个按键根本到不了网页，所有快捷键都会无声失效。设置页会对已知会被吞掉的组合给出警告。
